...
# Actual data: http://py4e-data.dr-chuck.net/comments_1995358.xml (Sum ends with 29)
# 
...
# syntax for details. 
# You could also work from the top of the XML down to the comments node and 
...
commentInfo = ET.fromstring(rawData)
commentCounts = commentInfo.findall('.//count')
print('Count:', len(commentCounts))
sum = 0
for commentCount in commentCounts:
    try:
        sum = sum + int(commentCount.text.strip())
    except:
        print(commentCount.text, ' is not a number.')
        sum = sum
print('Sum:', str(sum)+'...')
